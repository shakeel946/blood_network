...
def delete_donor(donor_id: int, _: User = Depends(admin_user), db: Session = Depends(get_db)):
    donor = get_donor_by_id(db, donor_id)
    if not donor:
        raise HTTPException(404, "Donor not found")

    # Remove the local profile photo too, when possible.
    if donor.profile_photo and donor.profile_photo.startswith("/uploads/"):
        photo_path = UPLOAD_DIR / Path(donor.profile_photo).name
        try:
            photo_path.unlink(missing_ok=True)
        except OSError:
            pass

    # Deleting the User cascades to the donor profile and donation history
    # through the SQLAlchemy relationship configured on User.donor_profile.
